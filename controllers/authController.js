...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 2) Validate and verify the token
  ...
  if (!currentUser) {
    next(
      new AppError(
        'The user belonging to this token does not longer exits',
        401
      )
    );
  }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  // validateBeforeSave is extremely helpful
  ...
  ${resetURL} \nIf you didn't forget the password, pls ignore this email.`;
  try {
    await sendEmail({
      email: user.email,
    ...
    res
      .status(200)
      .json({ status: 'success', message: 'Token send the email!' });
  } catch (err) {
